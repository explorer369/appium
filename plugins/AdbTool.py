...
class AdbTools(object):
    ...
    def screenshot(self, target_path=''):
        ...
        self.shell('screencap -p /sdcard/%s.png' % (Testtime,))
        time.sleep(1)
        if target_path == '':
            self.pull('/sdcard/%s.png' % (Testtime,), os.path.expanduser('~'))
        else:
            self.pull('/sdcard/%s.png' % (Testtime,), target_path)
        self.remove('/sdcard/%s.png' % (Testtime,))
            ...
            while 1:
                ...
                if num <= timeout:
                    if os.path.exists(os.path.join(os.path.expanduser('~'), 'base.apk')):
                        os.rename(os.path.join(os.path.expanduser('~'), 'base.apk'),
                                  os.path.join(os.path.expanduser('~'), '%s.apk' % package))
    def getCodeFromSms(arg,timeout=3):
        #获取手机里的短信验证码
        os.system("adb logcat -c")    #清空所有的日志缓存信息
        cmd = ' adb logcat -d |findstr D/Mms/Txn'   #设试失败待调试
        n = 0
        while n < timeout:

...
def Setting_device(self,automationName='Appium'):
        ...
        #desired_caps['appPackage'] = 'com.ss.android.article.news' # 今日头条的
        #desired_caps['appActivity'] = 'com.ss.android.article.news.activity.MainActivity'
        desired_caps['appPackage'] = 'com.zxsf.mortgage' # 此处手动填写，如想自动获取请在getDevices下设置要获取的APK路径和开启52行
        desired_caps['appActivity'] = '.ui.activitys.common.SplashActivity'
...
def get_element_screen_OCR(self,method=None,TEXT=None,number=0,name="ImageOnlineRecognition"):
    '''截取控件图片在线识别'''
    #用法：SettingDevice.get_element_screen_OCR(self,"id","com.wxws.myticket:id/etBecity")
    element = get_element(self,method,TEXT,number)
    self.extend.get_screenshot_by_element(element).write_to_file("D:\\screen", name)
    self.assertTrue(os.path.isfile("D:\\screen\\%s.png"%name))
    logger.info("+PASS+  截图成功！保存在D:\\screen\\%s.png"%name)
    text_my = OCR.picture_text(screen_path=r"D:\\screen\\%s.png"%name)  #把截图拿去在线识别
    return text_my
